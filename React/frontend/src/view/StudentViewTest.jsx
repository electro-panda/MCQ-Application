    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const result = await axios.get(`${path}/result/${user._id}`, { withCredentials: true });
                const attemptedTests = result.data.map((attempt) => attempt.test._id)
                setAttempt(result.data)
                setResult(attemptedTests)
            } catch (err) {
                console.error(err);
                        ...
                        <table className="min-w-full bg-white divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredTests.map((test) => {
                                    ...
                                    const currentAttempt = attempt.find(a => a.test._id === test._id);
                                    return (
                                        <tr key={test._id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                                {result.includes(test._id) ? <button
                                                    className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm"
                                                    onClick={() => navigate(`/result/detail/${currentAttempt._id}`)}>
                                                    View Result
                                                </button> : <button
